...
function forward_propagation(X, parametres) {
    ...
    for (let c = 1; c <= C; c++) {
        let Z = (parametres['W' + c] * activations['A' + (c-1)]) + parametres['b' + c];
        activations['A' + c] = 1 / (1 + Math.exp(-Z));
    }
    return activations;
}
...
function back_propagation(y, parametres, activations) {
    ...
    let gradients = {};
    for (let c = C; c >= 1; c--) {
        gradients['dW' + c] = (dZ * math.dot(dZ, math.transpose(activations['A' + (c - 1)])) / m);
        gradients['db' + c] = (dZ + 1) / m;
        if (c > 1) {
            dZ = ((math.dot(math.transpose(parametres['W' + c]), dZ) * math.subtract(1, activations['A' + (c - 1)])) * activations['A' + (c - 1)]);
        }
    }
    return gradients;
}
...
function update(gradients, parametres, learning_rate) {
    let C = Object.keys(parametres).length / 2;
    for (let c = 1; c <= C; c++) {
        parametres['W' + c] = math.subtract(parametres['W' + c], (gradients['dW' + c] *  learning_rate));
        parametres['b' + c] = math.subtract(parametres['b' + c], (gradients['db' + c] *  learning_rate));
    }
    return parametres;
}
...
function deepNeuralNetwork(X, y, hiddenLayers = [16, 16, 16], learningRate = 0.001, nIter = 3000) {
    ...
    var dimensions = [X.length].concat(hiddenLayers);
    dimensions.unshift(2); // dimension of X
    dimensions.push(1); // dimension of y
